...
int main(int argc, char *argv[])
{
	try
	{
		...
		if (argc==4&&(atoi(argv[3])==0&&argv[3][0]!='0' ) )
		{
		...
		for (int i=0; i<num_species; i++)
		{
			...
			if (!program_file)
			{
				cout<<"Cannot open file "<<program_file_name<<"!"<<endl;
				program_file.close();
				return 0;
			}
		...
		{
			cout<<"The grid height is illegal!"<<endl;
			return 0;
		}
		if (world_width>MAXWIDTH||world_width<1)
		{
			cout<<"The grid width is illegal!"<<endl;
			return 0;
		}
		int i_world=0;
		...
		for (int i=0;i<num_creatures;i++)
		{
			...
			{
				cout<<"Direction "<<direction_creature<<" is not recognized!"<<endl;
				return 0;
			}
